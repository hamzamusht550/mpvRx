...
object MpvHelpData {
    ...
    private val configVideo = listOf(
        option("hwdec", "<auto|no|yes|mediacodec-copy>", "Hardware decoding method. On Android, mediacodec-copy is the only reliable option.", true, "mediacodec-copy is preferred; mediacodec (no-copy) may fail on some devices"),
        option("hwdec-codecs", "<codec-list>", "Codecs to decode with hwdec (e.g. h264,hevc,vp8,vp9,mpeg2,vp9).", true, "On Android, typically all codecs go through MediaCodec anyway"),
        option("vo", "<driver>", "Video output driver. On Android, only 'gpu' (or 'gpu-next' on some forks) works.", true, "Only 'gpu' (or 'gpu-next' via mpv-android)"),
        option("gpu-api", "<auto|opengl|vulkan>", "GPU API for rendering. Android only supports OpenGL ES.", true, "Only opengl works on Android; vulkan is not supported"),
        option("gpu-context", "<auto|android>", "GPU context provider. Android auto-detects.", true),
        option("gpu-hwdec-interop", "<auto|vaapi-egl|d3d11|mediacodec>", "GPU interop for hardware decoding. Android uses mediacodec.", true),
        option("profile", "<name>", "Apply an mpv profile. Built-in: 'gpu-hq'."),
        option("scale", "<filter>", "Video upscaler. Common: bilinear, bicubic_fast, ewa_lanczossharp, mitchell, spline36, oversample."),
        option("cscale", "<filter>", "Chroma upscaler. Same options as scale."),
        option("dscale", "<filter>", "Video downscaler. Same options as scale."),
        option("tscale", "<filter>", "Temporal (frame-rate) scaler. Used with interpolation. Common: oversample, linear, mitchell, catmull_rom."),
        ...
        option("linear-upscaling", "<yes|no>", "Use linear-light scaling for upscaling (default: no).", true, "May increase GPU load significantly"),
        option("interpolation", "<yes|no>", "Enable frame interpolation via tscale (default: no).", true, "High GPU cost; may not be smooth on all devices"),
        option("interpolation-threshold", "<number>", "Maximum ratio difference for interpolation (default 0.0 = always)."),
        option("video-sync", "<audio|display-resample|display-resample-vdrop|display-adrop|display-vdrop|dropt>", "Video/audio sync method."),
        ...
        option("vf", "<filter-list>", "Video filter chain (e.g. vf=vapoursynth=/path/to/script, vf=scale=w=1920:h=1080)."),
        ...
        option("hdr-compute-peak", "<yes|no>", "Compute HDR peak brightness per-frame (default: auto).", true, "May affect performance on slower devices"),
        ...
        option("target-peak", "<number>", "Display peak brightness in nits (default auto).", true, "Set to your display's max brightness"),
        option("target-prim", "<auto|bt.470m|bt.601|bt.709|bt.2020|apple|srgb>", "Target display color primaries.", true, "Most Android devices are bt.709 or srgb"),
        option("target-trc", "<auto|bt.1886|srgb|pq|hlg>", "Target display transfer curve.", true, "Most Android devices are srgb"),
        option("tone-mapping", "<auto|clip|mobius|reinhard|hable|bt.2390|gamma>", "HDR to SDR tone-mapping algorithm.", true, "bt.2390 recommended for Android"),
        option("tone-mapping-param", "<number>", "Tone-mapping parameter (algorithm-specific tuning)."),
        option("tone-mapping-mode", "<auto|rgb|luma>", "Tone-mapping mode: per-channel RGB or luma-only.", true, "luma may be faster on mobile GPUs"),
        ...
        option("gpu-dumb-mode", "<auto|yes|no>", "Force software rendering fallback mode.", true, "May help on Android devices with GPU issues"),
        ...
        option("video-reversal-buffer", "<yes|no>", "Buffer for reverse playback (default: no).", true, "Uses significant memory"),
    )

    private val configAudio = listOf(
        option("ao", "<driver>", "Audio output driver. On Android only 'opensles' or 'audiotrack' work.", true, "Only opensles (default) or audiotrack"),
        option("audio-device", "<name>", "Audio device to use (list with --audio-device=help)."),
        option("audio-exclusive", "<yes|no>", "Exclusive audio output mode (bypasses mixer).", true, "Android typically doesn't support exclusive mode"),
        ...
        option("softvol", "<no|yes|auto-detect>", "Use software volume control (deprecated in newer mpv).", true, "May be forced on if hardware mixer is unavailable"),
        ...
        option("audio-spdif", "<codec-list>", "Codecs to pass through via S/PDIF (e.g. ac3,dts).", true, "Not supported on Android"),
        option("audio-file", "<path>", "External audio file to use instead of the default."),
    )

    private val configSubs = listOf(
        ...
        option("sub-font-provider", "<auto|fontconfig|directwrite|none>", "Subtitle font provider.", true, "Android uses its own font resolution"),
        option("osd-ass-cc", "<yes|no>", "Enable ASS (libass) drawing code in OSD (default: yes)."),
    )

    private val configOsd = listOf(
    ...
    private val configPlayback = listOf(
    ...
    private val configScreenshots = listOf(
    ...
    private val configInput = listOf(
        ...
        option("input-media-keys", "<yes|no>", "Listen for multimedia key events.", true, "May not work on all Android configurations"),
        ...
        option("input-touch", "<yes|no>", "Enable touch input (default: yes).", true, "Crucial for Android touch interaction"),
        option("input-ts-deadzone", "<number>", "Touchscreen deadzone factor (default 0.0)."),
    )

    private val configScripting = listOf(
    ...
    private val cmdPlayback = listOf(
        command("seek", "<target> [flags]", "Seek to a position. Flags: relative, absolute, keyframes, exact, backward."),
    ...
    private val cmdProperties = listOf(
        ...
        command("change-list", "<property> <op> <value>", "Modify a list property (op: add, remove, toggle, set, clr, clr-all)."),
        command("toggle", "<property>", "Toggle a boolean property."),
    )

    private val cmdFiles = listOf(
    ...
    private val cmdScreenshots = listOf(
    ...
    private val cmdScripting = listOf(
    ...
    private val cmdMeta = listOf(
        command("run", "<cmd> [args...]", "Run an external command/process.", androidNote = "Subprocess execution may be restricted on Android"),
        command("subprocess", "<args...>", "Run subprocess (native argument form).", false, "Likely blocked by Android sandbox"),
    ...
    private val propPlayback = listOf(
    ...
    private val propPlaylist = listOf(
    ...
    private val propDisplay = listOf(
    ...
    private val propChapters = listOf(
    ...
    private val propOptions = listOf(
    ...
    private val jsCore = listOf(
        ...
        jsApi("mp.command_native_async", "mp.command_native_async(table, cb)", "Execute command asynchronously with callback."),
    ...
    private val jsCommands = listOf(
        jsApi("mp.add_key_binding", "mp.add_key_binding(key, name?, fn?, flags?)", "Add a key binding."),
        jsApi("mp.add_forced_key_binding", "mp.add_forced_key_binding(key, name?, fn?, flags?)", "Add a forced key binding (overrides built-in)."),
        ...
        jsApi("mp.register_script_message", "mp.register_script_message(name, fn)", "Register a script-message handler."),
        jsApi("mp.unregister_script_message", "mp.unregister_script_message(name)", "Unregister a script-message handler."),
        jsApi("mp.add_hook", "mp.add_hook(type, priority, fn)", "Add a lifecycle hook (type: on_unload, on_load, etc.)."),
    ...
    private val jsInput = listOf(
        jsApi("mp.input.get", "mp.input.get(config)", "Request text input from user (config: {type, prompt, default_text, num_of_chars})."),
        jsApi("mp.input.select", "mp.input.select(config)", "Show a selectable list to the user (config: {prompt, items, default})"),
    ...
    private val jsUtils = listOf(
        ...
        jsApi("mp.utils.get_user_path", "mp.utils.get_user_path(path)", "Expand mpv user path (~~/, ~~script/, ~~home/ etc)."),
        ...
        jsApi("mp.utils.subprocess", "mp.utils.subprocess(config)", "Run subprocess (config: {args, cancellable, max_size, ...}).", false, "Subprocess likely restricted by Android sandbox"),
        jsApi("mp.utils.subprocess_detached", "mp.utils.subprocess_detached(config)", "Run detached subprocess.", false, "Likely blocked on Android"),
        jsApi("mp.utils.compile_js", "mp.utils.compile_js(fname, content)", "Compile JavaScript string into a function."),
    )

    private val jsLogging = listOf(
    ...
    private val configOptions = configVideo + configAudio + configSubs + configOsd +
        configPlayback + configScreenshots + configInput + configScripting + configGeneral

    private val commands = cmdPlayback + cmdProperties + cmdFiles + cmdScreenshots +
        cmdOsd + cmdScripting + cmdFilters + cmdMeta

    private val properties = propPlayback + propAudio + propVideo + propSubs +
        propPlaylist + propDisplay + propChapters + propOsd + propOptions

    private val jsApi = jsCore + jsProps + jsCommands + jsOsd + jsInput +
    ...
    private fun option(
        ...
        androidCompatible: Boolean = true, androidNote: String? = null,
